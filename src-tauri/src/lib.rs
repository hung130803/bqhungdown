...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // DỌN MẢNH TẢI DỞ trên MỌI thư mục kênh, LẶP LẠI mỗi 2 phút.
            //
            // VÌ SAO PHẢI LẶP (lỗi thật): bản trước chỉ dọn ĐÚNG MỘT LẦN lúc mở
            // app, còn vòng lặp 2 phút trong queue.rs thì chỉ quét
            // `default_folder` — KHÔNG quét thư mục riêng của từng kênh. Máy anh
            // Hùng để app chạy cả ngày với ~100 kênh lưu riêng thư mục → rác
            // sinh ra sau lúc mở app không ai dọn, dồn lại thành hàng chục file
            // rời rạc trong thư mục kênh.
            //
            // `sweep_junk_all` là TẬP CHA: gồm `default_folder` + thư mục từng
            // kênh + `watch_root` (xem `watcher::watch_folders`), nên nó thay
            // luôn vòng lặp cũ chỉ quét thư mục mặc định.
            {
                let q = queue.clone();
                ...
                std::thread::spawn(move || {
                    ...
                    loop {
                        ...
                        if n > 0 {
                            eprintln!("[junk] đã dọn {n} file mảnh tải dở");
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::set_watched_dest_dir,
            commands::set_watched_picked,
